...
def create_coco_labels(img_dir, json_dir, out_dir, phase="train", overwrite=False):
    ...
    for image_id, base_name in enumerate(tqdm(sorted(common_names), desc="[create_coco_labels] 이미지 처리 중")):
        ...
        try:
            ...
            if img is None:
                print(f"[create_coco_labels] 경고: 이미지를 로드할 수 없음: {img_path}")
                error_count += 1
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out_dir", required=True, help="COCO 형식 라벨 출력 디렉토리")
    parser.add_argument("--phase", default="train", help="데이터셋 단계(train, val, test)")
